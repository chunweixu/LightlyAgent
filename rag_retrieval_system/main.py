...
def main():
    ...
    faiss_results = retrieve_faiss_results(query, top_k=top_k, struct_index=struct_index)

    logger.info("Step 5: Applying fusion ranker...")
    top_results = fusion_ranker([], faiss_results, top_k=top_k)
    logger.info(f"Fusion ranker produced top {top_k} results: {top_results}")

    logger.info("Step 6: Mapping and deduplicating results...")
    unique_nodes = map_and_deduplicate_results(top_results, struct_index)
